...
M.load_config = function()
  ...
  local setup_exists, setup = pcall(require, "custom.setup")

  if setup_exists then
    -- merge user config if it exists and is a table; otherwise display an error
    if type(setup) == "table" then
      config.mappings = M.remove_disabled_keys(setup.mappings, config.mappings)
      config = merge_tb("force", config, setup) or {}
    else
      error "setup must return a table!"
    end
  end
...
M.remove_disabled_keys = function(setup_mappings, default_mappings)
  if not setup_mappings then
    return default_mappings
  end

  -- store keys in a array with true value to compare
  local keys_to_disable = {}
  for _, mappings in pairs(setup_mappings) do
...
M.packer_sync = function(...)
  local git_exists, git = pcall(require, "lib.tools.git")
  local defaults_exists, defaults = pcall(require, "lib.tools.config")
  ...
  if git_exists and defaults_exists then
    ...
    if current_branch_name:match(defaults.snaps.base_snap_branch_name .. "(.+)" .. "$") then
      vim.api.nvim_echo({
        ...
        { "PackerSync" },
        {
          " on a SetupSnapshot. This will cause issues if Setup dependencies contain "
