...
M.get_example_setup = function()
  local setup_path = require("lib.tools.config").custom.default_setup_example_path

  -- store in data variable
  local data = utils.file("r", setup_path)

  if not data then
    print "Error: Could not read exmaple setup!"
    return false
  end
