local function snap_checkout()
  ...
  for i, snapshot in ipairs(snapshot_list) do
    ...
    vim.list_extend(
      ...
      {
        { "    [" .. tostring(i) .. "] " },
        { (snapshot == git.update_branch or "") .. snapshot, "Title" },
  ...
  if selection == "c" then
    echo(misc.list_text_replace(prompts.cancelled_action, "<ACTION>", "SetupSnapshotCheckout"))
    return
  end
  ...
  if current_branch_name == git.update_branch then
    -- make sure that setup.lua exists, if not create it as a copy of the example
