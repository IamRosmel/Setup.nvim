local function snap_create()
  ...
  local utils = require "lib.tools"
  local git = require "lib.tools.git"
  local misc = require "lib.tools.misc"
  local defaults = require "lib.tools.config"
  local prompts = require "lib.tools.prompts"
  ...
  -- @LeonHeidelbach on GitHub by creating an issue in the main Setup repo.
  ...
  if string.lower(name) == "c" or name == "" then
    echo(misc.list_text_replace(prompts.cancelled_action, "<ACTION>", "SetupSnapshotCreate"))
    return
  end

  -- make sure that setup.lua exists, if not create it as a copy of the example file
  local result = misc.ensure_file_exists(defaults.custom.default_setup_path, misc.get_example_setup())

  -- if the default custom config could not be created return
  if not result then
    echo(misc.list_text_replace(prompts.setup_file_not_created, "<FILE_PATH>", defaults.custom.default_setup_path))
    return false
  end
  ...
  -- create a temporary commit to add our changes in setup.lua to the git history
